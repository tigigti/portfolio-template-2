...
<div class="top-container">
	...
	<div class="header-nav">
		...
		<img alt="Angelos Ioannou Logo" class="logo-light" src="<?php echo(get_template_directory_uri() . "/img/logo_light.png");?>"/>
		...
		<a href="#kontakt" class="contact-icon show-mobile"><img alt="Mail icon" src="<?php echo(get_template_directory_uri() . "/img/mail-icon.png");?>"/></a>
	</div>
...
<section id="aboutMe">
	<div class="picture-container">
		<img alt="Profile Picture" class="image" src="<?php the_field('profile_picture');?>">
	</div>
		...
		<?php $queryProjects = new WP_Query( array( "category_name" => "leistung")); ?>
			<?php while( $queryProjects->have_posts()): $queryProjects->the_post(); ?>
				<div class="leistung-card">
					<div class="leistung-card__icon"><img alt="<?php the_field('leistung_subheader');?> Logo" src="<?php the_field("leistung_icon");?>"/></div>
		...
		<?php while( $queryProjects->have_posts()): $queryProjects->the_post(); ?>
			<div class="project-container">
				<div class="text-container">
					...
					<a rel="noopener" target="_blank" class="text-container__link" href="<?php the_field("project_link");?>">Besuchen</a>
				</div>

				<div class="image-container">
					<img alt="Desktop Image of the Project <?php the_field('project_title');?>" class="main-image" src="<?php the_field('project_image');?>">
					<img alt="Responsive Image of the Project <?php the_field('project_title');?>" class="responsive-image" src="<?php the_field('responsive_image');?>">
				</div>
			</div>
		<?php endwhile;?>
		<!-- necessary for custom loop -->
...
<section class="kontakt">
	<form class="form-container" id="kontaktContainer">
		...
		<div class="form-group">
				<input placeholder="Name" type="text" name="name" aria-label="Name" />
				<input placeholder="E-Mail" type="email" name="email" aria-label="E-Mail" />
		</div>
		<input placeholder="Projekt Name" type="text" name="project" aria-label="Projekt Name" />

		<label for="desc" style="margin-bottom: 8px;">Beschreibung</label>
		<textarea id="desc" name="desc" rows=6></textarea>

		<div id="responses"></div>
		<input type="submit" value="Abschicken" class="submit-btn" aria-label="Submit" />
	</form>
</section>
...
<section id="projekte">
	...
	<?php $querySmallProjects = new WP_Query( array( "category_name" => "small_project")); ?>
		<?php while ( $querySmallProjects->have_posts() ): $querySmallProjects->the_post(); ?>
			<a rel="noopener" target="_blank" href="<?php the_field("project_link"); ?>" class="small-project-container">
				<div class="small-project__header"><?php the_title(); ?></div>
